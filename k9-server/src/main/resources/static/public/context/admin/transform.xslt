<xsl:stylesheet xmlns:svg="http://www.w3.org/2000/svg"
	...
	<xsl:template name="entity">
		...
		<xsl:element name="{@type}">
			...
			<xsl:attribute name="k9-ui"><xsl:value-of
				select="@commands" /> <xsl:value-of select="$focus" /> <xsl:value-of
			...
			<xsl:if test="@type = 'document'">
				<xsl:attribute name="href"><xsl:value-of
					select="$id" />/content</xsl:attribute>
			</xsl:if>
			...
			<container class="left" id="userbox">
				<xsl:call-template name="entity">
					<xsl:with-param name="id">
						<xsl:value-of
							select="adl:links[adl:rel='self']/adl:href" />
					</xsl:with-param>
	...
	<xsl:template name="project-page">
		<container class="main">
			<container class="left" id="projectbox">
				...
				<container class="lower list" id="membersbox"
					k9-ui="AddMembers">
					<xsl:attribute name="subject-uri"><xsl:value-of
						select="@localId" /></xsl:attribute>
					<xsl:for-each
						select="./adl:content/adl:value[@type='member']">
						<pmember>
							<xsl:attribute name="id"><xsl:value-of
								select="@localId" /></xsl:attribute>
			...
			<container class="right grid" id="documentbox"
				...
				<xsl:attribute name="subject-uri"><xsl:value-of
					select="@localId" /></xsl:attribute>
				<xsl:for-each
					select="./adl:content/adl:value[@type='document']">
					<xsl:call-template name="entity">
						<xsl:with-param name="id">
							<xsl:value-of select="@localId" />
						</xsl:with-param>
	...
	<xsl:template name="document-page">
		<container class="main">
			<container class="left" id="projectbox">
				<xsl:for-each
					select="./adl:content/adl:value[@type='project']">
					<xsl:call-template name="entity">
						<xsl:with-param name="id">
							<xsl:value-of select="@localId" />
						</xsl:with-param>
			...
			<container id="revisionbox">
				<xsl:variable name="count">
					<xsl:value-of
						select="count(./adl:content/adl:value[@type='revision'])" />
				</xsl:variable>
				<xsl:attribute name="class">right list <xsl:if
					test="$count > 10">more</xsl:if></xsl:attribute>
				<xsl:for-each
					select="./adl:content/adl:value[@type='revision' and count(preceding::adl:value) &lt; 11]">
					<member>
	...
	<xsl:template match="/adl:entity[adl:type='project']">
		<xsl:call-template name="project-page" />
	</xsl:template>

	<xsl:template match="/adl:entity[adl:type='document']">
		<xsl:call-template name="document-page" />
	</xsl:template>

	<xsl:template match="/adl:entity[not(adl:type)]">
