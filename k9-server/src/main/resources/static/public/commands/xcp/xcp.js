	...
	return function(event, contextMenu) {
		...
		if (copyElements.length > 0) {
			...
			if (cutElements.length > 0) {
				contextMenu.addControl(event, "/public/commands/xcp/cut.svg", "Cut", (e2, selector) => {
					contextMenu.destroy();
					performCopy(Array.from(copyElements), Array.from(cutElements));
				});
			}
			
			contextMenu.addControl(event, "/public/commands/xcp/copy.svg", "Copy", (e2, selector) => {
				contextMenu.destroy();
		...
		if ((pasteElements.length > 0) && (pasteUris.length > 0)) {
			contextMenu.addControl(event, "/public/commands/xcp/paste.svg", "Paste", (e2, selector) => {
				contextMenu.destroy();
