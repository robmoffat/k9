...
public class DocumentController extends AbstractADLContentController<Document> {
	...
	/**
	 * Testing out thymeleaf templates...
	 */
	@RequestMapping(path = "/{documentId}/info", method= {RequestMethod.GET}) 
	public String info(@PathVariable("documentId") long id, Model m) throws Exception {
		Revision r = getCurrentRevision(id);
		m.addAttribute("main", r.getDocument());
		return "api/document";
	}
